...
import { Platform,Modal,View,StyleSheet,TouchableHighlight,Keyboard,Alert,Image,TouchableOpacity,Picker,ScrollView,BackHandler,AppState } from 'react-native';
...
class Buy extends Component{
    ...
    _handleAppStateChange = nextAppState => {
        if (this.state.appState.match(/inactive|background/) && nextAppState === "active") {
            ...
            console.log("App has come to the backgroubd!");
        }
        ...
        if(this.tentativas > 0){
            ...
            this.manager.startDeviceScan(null, null, (error, device) => {
                ...
                this.state.data.map(item=>{
                    if(item.id !== null && item.id !== undefined && item.id !== '' && item.id === this.state.bluetoothInit){
    ...
    scan(){
        if(this.tentativas > 0 && Platform.OS === 'ios'){
            this.tentativas = this.tentativas - 1;
            //this.initBluetooth('E0:E5:CF:79:2B:8C'); 
            if(this.state.bluetoothInit !== null && this.state.bluetoothInit !== undefined){
                this.initBluetooth(this.state.bluetoothInit); 
            }
        }else{
            Alert.alert('Ops! :( ','Erro ao se conectar com a máquina, tente novamente');
            ...
            if(this.state.autorizadora !== '' || this.state.novaautorizadora !== ''){
                if(this.state.codigoDeSeguraca !== '' || this.state.novocodigo !== ''){
                    ...
                    if(this.state.valorDoPagamento === null && this.state.valorDoPagamento === undefined){
                        valorDoPagamentoTratado = this.state.preco;
                    }else{
                        valorDoPagamentoTratado = this.state.valorDoPagamento;
                    }
                    ...
                    try {
                        ...
                        let valorDoPagamento = valorDoPagamentoTratado;
    ...
    async estorno(IdCompra){
        ...
        if(IdCompra !== null && IdCompra !== undefined){
            try {
                ...
                let cartaoId = this.state.cartao;
                let PedidoId = this.state.IdCompra;
                let codigoDeSeguraca = this.state.codigoDeSeguraca;
                let cartao = {
                    id: cartaoId,
                ...
                api.defaults.headers.Authorization = `Bearer ${this.props.token}`;
                const cadastrarcompra = await api.post('/v1/Cancelamentos/'+PedidoId,{
                    PedidoId,
